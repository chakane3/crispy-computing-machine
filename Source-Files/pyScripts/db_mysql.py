...
# )
# cursor = mydb.cursor()
# cursor.execute("CREATE TABLE products (name VARCHAR(255), caliber VARCHAR(255), price DOUBLE(6, 2), link VARCHAR(255), imgURL VARCHAR(255), aType VARCHAR(255), description TEXT(2500), aTime VARCHAR(255))")
# cursor.close()
# mydb.close()
...
def insert_varibles_into_table(name, caliber, price, link, imgURL, aType, description, aTime):
    try:
        ...
        mydb.commit()
        print("Record inserted successfully into product table")
...
for i in d['ammo_results']:
    formattedPrice = ("%.2f" % float(i['price']))
    timeStamp = x[:10]
    insert_varibles_into_table(i['name'], i['caliber'], formattedPrice, i['link'], i['imgURL'], i['type'], i['description'], timeStamp)
